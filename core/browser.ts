...
export async function navegador(mostrar: boolean): Promise<BrowserContext> {
  ...
  if (motor === 'firefox') {
    try {
      ...
      log.registrar('alerta', /Executable doesn't exist/i.test(msg) ? 'Firefox do Playwright não está instalado (rode "npx playwright install firefox" na pasta do AutoCV). Usando o Edge desta vez.' : `Não consegui abrir o Firefox (${msg}). Usando o Edge desta vez.`);
    }
  }
  if (!contexto) {
