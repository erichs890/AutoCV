...
async function candidatar(vaga: Vaga, dados: DadosCandidatura, log: Log): Promise<ResultadoCandidatura> {
  ...
  try {
    ...
    const bloqueio = await detectarBloqueio(page);
    if (bloqueio) return { status: 'erro', motivo: bloqueio === 'bloqueado' ? 'o Indeed bloqueou o acesso deste navegador (captcha/anti-robô); tente mais tarde' : 'o Indeed pediu verificação (captcha); resolva na janela do navegador e tente de novo', captura: await captura('indeed-bloqueio') };

    // A busca só traz um trecho da descrição; a página da vaga tem o texto completo
    const descricao = ((await page.locator(INDEED.descricao).first().textContent({ timeout: 8000 }).catch(() => '')) ?? '').replace(/\n{3,}/g, '\n\n').trim();
    ...
    if (!(await botao.isVisible().catch(() => false))) return { status: 'erro', motivo: 'a vaga não oferece mais "Candidatar-se facilmente" (vaga encerrada ou candidatura no site da empresa)', captura: await captura('indeed-sem-botao') };
    ...
    if (INDEED.login.test(page.url())) return { status: 'erro', motivo: 'faça login no Indeed (Plataformas › Indeed › Entrar): sem sessão o Indeed não abre a candidatura', captura: await captura('indeed-login') };
    await page.locator('input, textarea, button').first().waitFor({ state: 'visible', timeout: 20000 }).catch(() => {});

    const r = await executarFormulario(page, dados, log, { convencoes: INDEED.convencoes });
    const resumo: ResumoFormulario = { etapas: r.etapas.length, campos: r.etapas.reduce((s, e) => s + e.campos.length, 0), perguntas: r.perguntasRespondidas, typeform: r.typeform, incomum: r.etapas.length > 6 || r.etapas.some(e => e.campos.some(c => c.tipo === 'desconhecido')) };
