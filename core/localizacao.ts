...
const partesDe = (texto: string) => texto.split(/\s*[-,/|]\s*/).map(p => p.trim()).filter(Boolean);
...
export function lerLocal(texto: string): { cidade: string; uf: string } {
  ...
  const candidatos = partes.filter(p => !UFS.has(p.toUpperCase()) && !(p.length === 2 && ISO.has(p.toUpperCase()))).map(normalizar).filter(p => !NOME_DE_PAIS.has(p));
